...
export default function ManageProviderOrders() {
  const {
    data: data,
    isFetching,
    isLoading,
  ...
  const handleUpdateProductStatus = async (payload: any) => {
    try {
      const res = await updateProductStatus(payload).unwrap();
      if (res.success) toast.success("Product status updated successfully!");
      refetch();
    } catch (error) {
      console.error(error);
